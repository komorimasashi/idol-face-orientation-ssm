...
build_args_from_cfg <- function(cfg) {
  ...
  csv_path <- if (is.null(cfg$csv)) {
    file.path("..", "results", "forecast", axis_name, paste0("forecast_monthly_", axis_name, ".csv"))
  } else {
    cfg$csv
  }
...
run_plot_main(
  ...
  defaults = list(
    base_dir = script_dir,
    axis = axis_name,
    csv = file.path("..", "results", "forecast", axis_name, paste0("forecast_monthly_", axis_name, ".csv")),
    out = file.path("..", "results", "figures"),
