...
run_analysis_main <- function(args = commandArgs(trailingOnly = TRUE), defaults = list()) {
  ...
  stan_file <- resolve_path(get_arg_value(args, "--stan", defaults$stan %||% "nonhier_rw_forecast.stan"), base_dir, must_work = TRUE)
  out_dir <- resolve_path(get_arg_value(args, "--out", defaults$out %||% "../results/forecast"), base_dir, must_work = FALSE)
...
run_plot_main <- function(args = commandArgs(trailingOnly = TRUE), defaults = list()) {
  ...
  forecast_csv <- resolve_path(
    get_arg_value(args, "--csv", defaults$csv %||% "../results/forecast/pitch/forecast_monthly_pitch.csv"),
    base_dir,
    must_work = TRUE
  )
  out_dir <- resolve_path(get_arg_value(args, "--out", defaults$out %||% "../results/figures"), base_dir, must_work = FALSE)
