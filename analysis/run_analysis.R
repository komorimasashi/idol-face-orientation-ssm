...
build_args_from_cfg <- function(cfg, script_dir) {
  ...
  csv_path <- if (is.null(cfg$csv)) {
    file.path("..", "data", "ssm_inputs", paste0("ssm_input_", axis_name, ".csv"))
  } else {
    cfg$csv
  }
  out_path <- if (is.null(cfg$out)) {
    file.path("..", "results", "forecast", axis_name)
  } else {
    cfg$out
  }
...
run_analysis_main(
  ...
  defaults = list(
    base_dir = script_dir,
    csv = file.path("..", "data", "ssm_inputs", paste0("ssm_input_", axis_name, ".csv")),
    stan = "nonhier_rw_forecast.stan",
    out = file.path("..", "results", "forecast", axis_name),
    axis = axis_name,
    stem = axis_name
  )
)
